...
class Scraper:
    ...
    def __init__(self, number_of_items: int, item_types: list) -> None:
        """
        This scrapes totalwine.com for wine listings and is able to take demanded number of listings and a list of different types of wines to scrape.
    ...
    def get_response(self, page: int, wine_type: str) -> Union[requests.models.Response, str]:
        try:
    ...
    @staticmethod
    def get_categories(item: Dict[str, Any], id: int, regions: list, wine_types: list, varietals: list, appelations: list, styles: list, countries: list) -> list:
        """
        ...
        try:
            ...
            categories = ['REGION', 'PRODUCT_TYPE', 'VARIETAL_TYPE', 'APPELLATION', 'STYLE']
            ...
            for item in range(len(item_categories)):
                ...
                if item_categories[item]['type'] == 'REGION':
                    extracted_country = item_categories[item]['storefrontUrl'].split('/')[2]
                    item_country.extend([extracted_country])

            countries.extend(item_country) if len(item_country) != 0 and item_country != ['c'] else countries.extend([None])
